...
* (Encouraged) When iterating over a map with a range-based loop
  ...
  particularly well paired with local `key`and `value` aliases for `.first`and `.second` (often const-ref).
...
collection.
 ...
 The[Boost library collection](https://www.boost.org/) is a popular collection of
...
Naming rules are pretty arbitrary, but we feel that
...
Note that certain universally-known abbreviations are OK, such as`i` for an iteration variable and `T` for a
template parameter.
...
names, prefer to capitalize the abbreviations or acronyms as single words (i.e`StartRpc()`, not `StartRPC()`).

Template parameters should follow the naming style for their
category: type template parameters should follow the rules for[type names](#Type_Names), and non-type template
parameters should follow the rules for [variable names](#Variable_Names).
...
should end in `.inc` (see also the section on[self-contained headers](#Self_contained_Headers)).
...
Enumerators (for both scoped and unscoped enums) should be named *either* like[constants](#Constant_Names) or like[macros](#Macro_Names): either `kEnumName` or`ENUM_NAME`.

Preferably, the individual enumerators should be named
like [constants](#Constant_Names). However, it
is also acceptable to name them like[macros](#Macro_Names). The enumeration name,`UrlTableErrors` (and`AlternateUrlTableErrors`), is a type, and
...
of macros](#Preprocessor_Macros); in general macros should *not* be used.
...
next contributor who will need to
...
*/` syntax; however, `//` is*much* more common. Be consistent with how you
...
`TODO`s should include the string`TODO` in all caps, followed by the name, e-mail address, bug ID, or other
identifier
...
Coding style and formatting are pretty arbitrary, but a project is much easier to follow
...
some getting used to, but it is important that all project contributors follow the
style rules so that they can all read and understand
everyone's code easily.

To help you format code correctly, we've
created a[settings file for emacs](https://raw.githubusercontent.com/google/styleguide/gh-pages/google-c-style.el).
...
enhances readability — for example,`"\xEF\xBB\xBF"`, or, even more simply,`u8"\uFEFF"`, is the Unicode zero-width
no-break space character, which would be invisible if
included in the source as straight UTF-8.

Use the `u8` prefix
to guarantee that a string literal containing`\uXXXX` escape sequences is encoded as UTF-8.
...
not fit on a single line as you would wrap arguments in a[function call](#Function_Calls).
...
braces. Some projects require that an`if` must always have an accompanying
brace.
...
another must be annotated using the`ABSL_FALLTHROUGH_INTENDED;` macro (defined in `absl/base/macros.h`).`ABSL_FALLTHROUGH_INTENDED;` should be placed at a
...
When you have a boolean expression that is longer than the[standard line length](#Line_Length), be
...
the punctuation operators, such as`&&` and `~`, rather than
...
You may choose between `=`,`()`, and `{}`; the following are
all correct:
...
clean-up operation (preferably when no-one
